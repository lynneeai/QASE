...
class Finetune_Llama_Config:
    ...
    def __init__(self, dataset, base_model, save_checkpoint_after_steps=False):
        ...
        if save_checkpoint_after_steps:
            self.step_ckpt_dir = f"{self.output_dir}/step_checkpoint"
            if not os.path.exists(self.step_ckpt_dir):
                os.makedirs(self.step_ckpt_dir)
